...
// ReconcileStuckJobs marks stale in-flight jobs/variants as failed to prevent infinite limbo states.
// POST /api/v1/internal/jobs/reconcile-stuck?max_age_minutes=45
func ReconcileStuckJobs(c echo.Context) error {
	...
	maxAgeMinutes := parsePositiveInt(c.QueryParam("max_age_minutes"), 45)
	if maxAgeMinutes < 15 {
		maxAgeMinutes = 15
	}
