...
// AllowedModels is the set of valid FAL.AI video generation models.
// Exported so domain/brief.batch can import it.
...
// AllowedJobStatuses is the set of valid job status transitions.
...
func SubmitJob(c echo.Context) error {
	...
	// TODO: enqueue asynq scrape task to Railway Redis (TCP)
	return c.JSON(http.StatusAccepted, map[string]any{
		...
		"plan_tier":          planTier,
		"created_at":         util.TsTime(job.CreatedAt),
		"updated_at":         util.TsTime(job.UpdatedAt),
	})
}
...
func GetJob(c echo.Context) error {
	...
	parsedJobID, err := util.ParseUUID(jobID)
	if err != nil {
...
// UpdateJobStatus handles PATCH /api/v1/jobs/:id/status
func UpdateJobStatus(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, map[string]any{
		"job_id":      util.UUIDString(updated.ID),
		"org_id":      claims.OrgID,
...
func ListJobs(c echo.Context) error {
	...
	for _, row := range rows {
		jobs = append(jobs, map[string]any{
			"job_id":      util.UUIDString(row.ID),
			"org_id":      claims.OrgID,
			"status":      row.Status,
			"product_url": row.ProductUrl,
			"model":       row.Model,
			"created_at":  util.TsTime(row.CreatedAt),
			"updated_at":  util.TsTime(row.UpdatedAt),
		})
	}
	return c.JSON(http.StatusOK, map[string]any{
		"jobs":        jobs,
...
// StreamJob handles GET /api/v1/jobs/:id/stream — SSE endpoint
