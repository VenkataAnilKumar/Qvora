...
// InitiateSignalOAuth returns the platform OAuth redirect URL for the calling org.
...
// Public — auth is derived from the signed state token.
// ---------------------------------------------------------------------------

// HandleSignalOAuthCallback completes the OAuth code exchange and persists the connection.
func HandleSignalOAuthCallback(c echo.Context) error {
	...
	workspaceID, wsErr := store.WorkspaceIDForOrg(c.Request().Context(), orgID)
	if wsErr != nil {
